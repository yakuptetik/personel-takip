...
function handleLogin(){
  ...
  .then(() => {
    const redirectName = route.query.redirect ? route.query.redirect : 'Home';
...
<template>
<section class=" flex items-center justify-between h-[100%]">
  <div class="flex w-full items-center justify-center px-6 py-8 mx-auto h-full lg:py-0">
      <div class="w-full bg-gray-50 rounded-lg border  border-gray-300 md:mt-0 sm:max-w-md xl:p-0">
              ...
              <form @submit.prevent="handleLogin()" class="space-y-4 md:space-y-6">
                  <div class="flex flex-col">
                    ...
                    <input 
                        ...
                        class="disabled:bg-slate-50 disabled:text-white disabled:border-slate-200 disabled:shadow-none
                              invalid:border-indigo-500 invalid:text-gray-900  focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500
                              focus:invalid:border-indigo-500 focus:invalid:ring-indigo-500  border border-gray-300 text-gray-900 sm:text-sm
                  ...
                  <div>
                      ...
                      <input 
                        ...
                        id="password"
                        class="disabled:bg-slate-50 disabled:text-white disabled:border-slate-200 disabled:shadow-none
                              invalid:border-indigo-500 invalid:text-indigo-600  focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500
                              focus:invalid:border-indigo-500 focus:invalid:ring-indigo-500 border border-gray-300 text-gray-900 sm:text-sm
                              rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                  </div>
                  <button :disabled='isDisabled' type="submit" class="cursor-pointer w-full bg-indigo-500 hover:bg-indigo-600  rounded-lg h-10 ">
                    <div class=" px-2 py-1 text-white ">
                      <template v-if="isloading">
                        <div role="status" class="flex items-center justify-center text-white text-md ">
                          ...
                          <span class=" text-white pl-3">Login</span>
                        </div>
                      </template>

                      <template v-else>
                        <div role="status" class="flex items-center justify-center text-white text-md pl-3">
                          <span class=" text-white h-5 flex items-center">Login</span>
                        </div>
                      </template>
                    </div>
                  </button>  
  ...
  <div class="w-full max-lg:hidden h-full bg-indigo-500">
    <div class="hidden md:block w-full h-full py-10 px-10">
