...
<template>
    <div class="bg-gray-100 w-full p-10">
        <div class="flex items-center justify-end">
            <button @click="isModalShow = true" class="bg-blue-500 hover:bg-blue-400 text-white border  px-3 py-2 rounded-lg uppercase text-sm">
              Add projects
            </button>          
        </div>
        <div class="mt-4 h-full">
            <div class="uppercase text-2xl py-2 px-3 bg-gray-200 border border-b-transparent rounded-t-xl">
              projects
            </div>
            <div class=" w-full bg-white  border overflow-y-auto">
              <div class=" overflow-y-auto" v-for="project in projects">
                <div class="p-3 text-lg uppercase w-full hover:bg-gray-50 border-b flex items-center justify-between group">
                  <div>
                    {{ project.name }}
                  </div>
                  <div class="flex items-center gap-2">
                      <div class="group-hover:opacity-100 opacity-0 cursor-pointer">
      ...
      <AddProjectModal v-if="isModalShow" @close-modal="isModalShow = false"/>
    </div>
</template>